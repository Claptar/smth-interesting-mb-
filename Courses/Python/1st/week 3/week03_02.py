...
class Truck(CarBase):
    def __init__(self, brand, photo_file_name,  carrying, body_whl):
        ...
        if all(is_digit(x) for x in body_whl) and all(float(x) > 0 for x in body_whl) and len(body_whl) == 3:
...
def get_car_list(csv_filename):
    ...
    for line in lines[1:]:
        ...
        if len(line) < 7 or os.path.splitext(line[3])[1] not in ['.png', '.jpeg', '.jpg', '.gif'] or not is_digit(line[5]):
            continue
        elif line[0] == 'car':
            del line[6], line[4], line[0]
            if '' not in line and line[1].isdigit():
                car_list.append(Car(line[0], line[2], line[3], line[1]))
            else:
                continue
        elif line[0] == 'truck':
